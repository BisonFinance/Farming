...
contract BISONFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BISON
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate,
        ...
        require(_withdrawLockPeriod <= 200 days, "withdraw lock must be less than 90 days");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate,
        uint256 _withdrawLockPeriod ) external onlyOwner {
        require(_withdrawLockPeriod <= 200 days, "withdraw lock must be less than 200 days");
        if (_withUpdate) {
            massUpdatePools();
        }
